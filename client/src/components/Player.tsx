...
interface PlayerProps {
  ...
  isDesignMode?: boolean; // Prop to disable pointer lock in design mode
}
...
export const Player: React.FC<PlayerProps> = ({
  ...
  isDesignMode = false, // Destructure with default false
}) => {
  ...
  const calculateClientMovement = useCallback(
    (
      ...
      if (cameraMode === CAMERA_MODES.FOLLOW) {
        ...
        console.log(
          `[Orbital Move Calc] Mode: ORBITAL, Using fixed yaw: ${rotationYaw.toFixed(
            3
          )}`
        );
      }
      ...
      worldMoveVector = localMoveVector
        .clone()
        .applyAxisAngle(new THREE.Vector3(0, 1, 0), rotationYaw);
      ...
      if (cameraMode === CAMERA_MODES.ORBITAL) {
        console.log(
          `[Orbital Move Calc] Input: F${inputState.forward ? 1 : 0} B${
            inputState.backward ? 1 : 0
          } L${inputState.left ? 1 : 0} R${
            inputState.right ? 1 : 0
          }, MoveVector: (${worldMoveVector.x.toFixed(
            2
          )}, ${worldMoveVector.z.toFixed(2)}), Delta: ${delta.toFixed(4)}`
        );
      }
    ...
    [cameraMode]
  ); // Depend on cameraMode from state
    ...
    loader.load(
      ...
      (fbx) => {
        ...
        if (group.current) {
          ...
          try {
            console.log(
              `[Player Model Effect ${playerData.username}] Traversing loaded FBX to find embedded lights...`
            );
            fbx.traverse((child) => {
              if (child && child instanceof THREE.Light) {
                ...
                console.log(
                  `[Player Model Effect ${
                    playerData.username
                  }] --- FOUND AND REMOVING EMBEDDED LIGHT --- Name: ${
                    child.name || "Unnamed"
                  }, Type: ${child.type}`
                );
            ...
            console.error(
              `[Player Model Effect ${playerData.username}] Error during fbx.traverse for light removal:`,
              traverseError
            );
          }
      ...
      (error: any) => {
        console.error(
          `[Player Model Effect ${playerData.username}] Error loading model ${mainModelPath}:`,
          error
        );
      }
    );
  ...
  const loadAnimations = (mixerInstance: THREE.AnimationMixer) => {
    ...
    const basePath =
      characterClass === "Paladin" ? "/models/paladin/" : "/models/wizard/";
    ...
    const animKeys = {
      idle:
        characterClass === "Wizard"
          ? "wizard-standing-idle.fbx"
          : "paladin-idle.fbx",
      "walk-forward":
        characterClass === "Wizard"
          ? "wizard-standing-walk-forward.fbx"
          : "paladin-walk-forward.fbx",
      "walk-back":
        characterClass === "Wizard"
          ? "wizard-standing-walk-back.fbx"
          : "paladin-walk-back.fbx",
      "walk-left":
        characterClass === "Wizard"
          ? "wizard-standing-walk-left.fbx"
          : "paladin-walk-left.fbx",
      "walk-right":
        characterClass === "Wizard"
          ? "wizard-standing-walk-right.fbx"
          : "paladin-walk-right.fbx",
      "run-forward":
        characterClass === "Wizard"
          ? "wizard-standing-run-forward.fbx"
          : "paladin-run-forward.fbx",
      "run-back":
        characterClass === "Wizard"
          ? "wizard-standing-run-back.fbx"
          : "paladin-run-back.fbx",
      "run-left":
        characterClass === "Wizard"
          ? "wizard-standing-run-left.fbx"
          : "paladin-run-left.fbx",
      "run-right":
        characterClass === "Wizard"
          ? "wizard-standing-run-right.fbx"
          : "paladin-run-right.fbx",
      jump:
        characterClass === "Wizard"
          ? "wizard-standing-jump.fbx"
          : "paladin-jump.fbx",
      attack1:
        characterClass === "Wizard"
          ? "wizard-standing-1h-magic-attack-01.fbx"
          : "paladin-attack.fbx",
      cast:
        characterClass === "Wizard"
          ? "wizard-standing-2h-magic-area-attack-02.fbx"
          : "paladin-cast.fbx",
      damage:
        characterClass === "Wizard"
          ? "wizard-standing-react-small-from-front.fbx"
          : "paladin-damage.fbx",
      death:
        characterClass === "Wizard"
          ? "wizard-standing-react-death-backward.fbx"
          : "paladin-death.fbx",
    };
    ...
    Object.entries(animationPaths).forEach(([name, path]) => {
      ...
      fetch(path)
        .then((response) => {
          if (!response.ok) {
            console.error(
              `Animation file not found: ${path} (${response.status})`
            );
            loadedCount++;
        ...
        .catch((error) => {
          console.error(
            `Network error checking animation file ${path}:`,
            error
          );
          loadedCount++;
          checkCompletedLoading();
    ...
    const checkCompletedLoading = () => {
      ...
      if (loadedCount === totalCount) {
        ...
        if (successCount === totalCount) {
          ...
          console.warn(
            `⚠️ Loaded ${successCount}/${totalCount} animations. Some might be missing.`
          );
        }
        ...
        console.log(
          ...
          Object.keys(newAnimations).join(", ")
        );
        ...
        if (newAnimations["idle"]) {
          ...
          console.error(
            "Idle animation not found among loaded animations! Player might not animate initially."
          );
        }
      }
    };
    ...
    const loadAnimationFile = (
      name: string,
      path: string,
      mixerInstance: THREE.AnimationMixer
    ) => {
      if (!mixerInstance) {
        console.error(
          `Cannot load animation ${name}: mixer is not initialized`
        );
      ...
      loader.load(
        ...
        (animFbx) => {
          try {
            ...
            console.log(
              `Animation "${name}" loaded. Duration: ${clip.duration}s, Tracks: ${clip.tracks.length}`
            );
        ...
        (error: any) => {
          console.error(
            `Error loading animation ${name} from ${path}: ${
  ...
  const makeAnimationInPlace = (clip: THREE.AnimationClip) => {
    ...
    const positionTracks = tracks.filter((track) =>
    ...
    rootTrack = positionTracks.find((track) =>
      rootNames.some((name) =>
  ...
  const retargetClip = (clip: THREE.AnimationClip, sourceModelPath: string) => {
    ...
    const sourceFileName =
      sourceModelPath.split("/").pop()?.split(".")[0] || "";
  ...
  const playAnimation = useCallback(
    (name: string, crossfadeDuration = 0.3) => {
  ...
  ); // Add mixer to dependencies

  // --- NEW Effect: Explicitly set shadow props when model is loaded ---
  useEffect(() => {
    if (model && group.current) {
      console.log(
        `[Player Shadow Effect ${playerData.username}] Model loaded, traversing group to set shadow props on meshes.`
      );
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (cameraMode === CAMERA_MODES.FOLLOW) {
        ...
        localRotationRef.current.y =
          THREE.MathUtils.euclideanModulo(
            localRotationRef.current.y + Math.PI,
            2 * Math.PI
          ) - Math.PI;
        ...
        orbital.elevation = Math.max(
          Math.PI / 12,
          Math.min(Math.PI / 2.1, orbital.elevation)
        );
      }
    };
    ...
    const handleMouseWheel = (e: WheelEvent) => {
      ...
      if (cameraMode === CAMERA_MODES.FOLLOW) {
        ...
        targetZoom.current = Math.max(
          minZoom,
          Math.min(maxZoom, zoomLevel.current + zoomChange)
        );
      } else if (cameraMode === CAMERA_MODES.ORBITAL) {
        // Orbital camera zoom
        const orbital = orbitalCameraRef.current;
        ...
        orbital.distance = Math.max(
          3,
          Math.min(20, orbital.distance + zoomChange)
        );
      }
    };

    // Request pointer lock on click (but not in design mode)
    const handleCanvasClick = () => {
      if (!isPointerLocked.current && !isDesignMode) {
  ...
  const toggleCameraMode = useCallback(() => {
    ...
    if (newMode === CAMERA_MODES.ORBITAL) {
      ...
      orbitalCameraRef.current.playerFacingRotation =
        localRotationRef.current.y;
      console.log(
        `[Orbital Toggle] Storing playerFacingRotation: ${orbitalCameraRef.current.playerFacingRotation.toFixed(
          3
        )}`
      ); // DEBUG
      ...
      console.log(
        `Entering orbital mode. Stored player rotation: ${(
          localRotationRef.current.y *
          (180 / Math.PI)
        ).toFixed(2)}°`
      );
  ...
  useFrame((state, delta) => {
    {
      ...
      if (group.current && modelLoaded) {
        if (isLocalPlayer && currentInput) {
          ...
          const serverPosition = new THREE.Vector3(
            dataRef.current.position.x,
            dataRef.current.position.y,
            dataRef.current.position.z
          );
          ...
          const positionError = localPositionRef.current.distanceTo(
            unflippedServerPosition
          );
          ...
          if (positionError > POSITION_RECONCILE_THRESHOLD) {
            ...
            if (cameraMode !== CAMERA_MODES.ORBITAL) {
              localPositionRef.current.lerp(
                serverPosition,
                RECONCILE_LERP_FACTOR
              );
            }
          }

          // 2.5 RECONCILIATION (Rotation)
          const serverRotation = new THREE.Euler(
            0,
            dataRef.current.rotation.y,
            0,
            "YXZ"
          );
          const reconcileTargetQuat = new THREE.Quaternion().setFromEuler(
            serverRotation
          );
          ...
          if (cameraMode === CAMERA_MODES.FOLLOW) {
            ...
            if (isMovingDiagonally) {
              ...
              const localMoveVector = new THREE.Vector3(
                localMoveX,
                0,
                localMoveZ
              ).normalize();
              const cameraYaw = localRotationRef.current.y;
              const worldMoveDirection = localMoveVector.applyAxisAngle(
                new THREE.Vector3(0, 1, 0),
                cameraYaw
              );

              // Calculate the base target yaw
              targetVisualYaw = Math.atan2(
              ...
              if (forward && !backward) {
                // Check if primary movement is forward
          ...
          const targetVisualRotation = new THREE.Euler(
            0,
            targetVisualYaw,
            0,
            "YXZ"
          );
          const targetVisualQuat = new THREE.Quaternion().setFromEuler(
          ...
          if (isDebugArrowVisible && scene) {
            ...
            if (debugArrowRef.current) {
              ...
              debugArrowRef.current.position
                .copy(playerWorldPos)
                .add(new THREE.Vector3(0, 0.5, 0)); // Adjust origin height
            ...
            if (debugArrowRef.current && debugArrowRef.current.parent) {
              console.log(
                "[Debug Arrow] Removing arrow (prop is false or no scene)."
              ); // Log removal
          ...
          // Not the local player anymore or initially
      ...
      if (isLocalPlayer && group.current) {
        ...
        if (cameraMode === CAMERA_MODES.FOLLOW) {
          zoomLevel.current +=
            (targetZoom.current - zoomLevel.current) * Math.min(1, dt * 6);
        }
        ...
        if (cameraMode === CAMERA_MODES.FOLLOW) {
          ...
          const lookTarget = playerPosition
            .clone()
          ...
          const horizontalDistance =
            orbital.distance * Math.cos(orbital.elevation);
          const height = orbital.distance * Math.sin(orbital.elevation);

          // Use orbital.angle for camera rotation, playerPosition for center
          const orbitX =
            playerPosition.x + Math.sin(orbital.angle) * horizontalDistance;
          const orbitY = playerPosition.y + height;
          const orbitZ =
            playerPosition.z + Math.cos(orbital.angle) * horizontalDistance;
  ...
  useEffect(() => {
    // Explicitly wrap hook body
    {
      ...
      if (
        serverAnim &&
        serverAnim !== currentAnimation &&
        animations[serverAnim]
      ) {
        ...
        try {
          ...
          console.error(
            `[Anim Error] Error playing animation ${serverAnim}:`,
            error
          );
    ...
    playAnimation,
    currentAnimation,
  ]); // Dependencies include things that trigger animation changes
