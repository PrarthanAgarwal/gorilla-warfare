...
export const DebugPanel: React.FC<DebugPanelProps> = ({
    ...
    modelTypes.forEach((type) => {
      ...
      modelPromises.push(
        fetch(mainPath)
          .then((response) => {
            results.push(
              `${mainPath}: ${response.ok ? "OK" : "MISSING"} (${
                response.status
              })`
            );
            return response;
      ...
      ["idle", "walk-forward", "attack"].forEach((anim) => {
        const animPath = `/models/${type}/${type}-${
          anim === "attack" && type === "wizard"
        ...
        modelPromises.push(
          fetch(animPath)
            .then((response) => {
              results.push(
                `${animPath}: ${response.ok ? "OK" : "MISSING"} (${
                  response.status
                })`
              );
              return response;
  ...
  const localPlayerDisplay = localPlayer
    ? `${localPlayer.username} (${localPlayer.characterClass}) (ID: ${identity
        ?.toHexString()
        .substring(0, 8)}) HP:${localPlayer.health}`
    : "N/A";
  ...
  return (
    ...
    >
      ...
      {isPointerLocked && (
        <div
          style={{ color: "#ffcc00", fontSize: "10px", marginBottom: "5px" }}
        >
        ...
        <>
          ...
          {localPlayer && (
            <div style={{ marginTop: "10px" }}>
              ...
              <div>
                Position: ({Math.round(localPlayer.position.x)},{" "}
              ...
              <div>
                ...
                <span style={{ color: "#ffcc00" }}>
                  {localPlayer.currentAnimation || "none"}
                </span>
              </div>
            </div>
          )}
            ...
            >
              {Array.from(playerMap.values()).map((player) => (
                <li key={player.identity.toHexString()}>
                  {player.username} ({player.characterClass}) -{" "}
                  {player.identity.toHexString().substring(0, 8)}...
                  {player.currentAnimation && (
                    <span style={{ color: "#a0e0ff" }}>
                      {" "}
                      [{player.currentAnimation}]
                    </span>
                  )}
          ...
          <div style={{ marginTop: "10px" }}>
            <button
              ...
              style={{
                marginRight: "10px",
                backgroundColor: showControls ? "#555" : "#4a54df",
                padding: "5px 10px",
                color: "white",
                border: "none",
                borderRadius: "3px",
