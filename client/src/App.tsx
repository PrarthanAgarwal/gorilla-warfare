...
function App() {
  ...
  const [players, setPlayers] = useState<ReadonlyMap<string, PlayerData>>(
    new Map()
  );
  ...
  const [isDesignMode, setIsDesignMode] = useState(false); // State for design mode

  // Design mode state management
  const designModeState = useDesignModeState(isDesignMode, (objects) => {
    console.log("Layout saved:", objects);
  });
  ...
  const playerRotationRef = useRef<THREE.Euler>(
  ...
  const registerTableCallbacks = useCallback(() => {
    ...
    conn.db.player.onInsert((_ctx: EventContext, player: PlayerData) => {
      ...
      if (
        identity &&
        player.identity.toHexString() === identity.toHexString()
      ) {
    ...
    conn.db.player.onUpdate(
      (_ctx: EventContext, _oldPlayer: PlayerData, newPlayer: PlayerData) => {
        setPlayers((prev: ReadonlyMap<string, PlayerData>) => {
        ...
        if (
          identity &&
          newPlayer.identity.toHexString() === identity.toHexString()
        ) {
  ...
  const handleDelegatedClick = useCallback((event: MouseEvent) => {
    ...
    if (button) {
      ...
      console.log(
        `[CLIENT] Button click detected: ${button.getAttribute("data-action")}`
      );
  ...
  const keyMap: {
    [key: string]: keyof Omit<InputState, "sequence" | "castSpell">;
  } = {
    KeyW: "forward",
    KeyS: "backward",
    KeyA: "left",
  ...
  const sendInput = useCallback(
    (currentInputState: InputState) => {
      ...
      for (const key in currentInputState) {
        if (
          currentInputState[key as keyof InputState] !==
          lastSentInputState.current[key as keyof InputState]
        ) {
          changed = true;
          break;
        }
      ...
      if (
        changed ||
        currentInputState.sequence !== lastSentInputState.current.sequence
      ) {
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      // Don't handle player input in design mode
  ...
  const handleMouseDown = useCallback(
    (event: MouseEvent) => {
      ...
      if (event.button === 0) {
        ...
        // Request pointer lock when clicking in play mode
        try {
    ...
    (event: MouseEvent) => {
      ...
      if (document.pointerLockElement === document.body) {
        ...
        playerRotationRef.current.x = Math.max(
          ...
          Math.min(
            Math.PI / 2.5,
            playerRotationRef.current.x - event.movementY * sensitivity
          )
        );
      }
    },
  ...
  const setupInputListeners = useCallback(() => {
    if (isDesignMode) return; // Don't set up input listeners in design mode
  ...
  useEffect(() => {
    const gameLoop = () => {
      if (!connected || !conn || !identity || isDesignMode) {
    ...
    if (connected && !animationFrameIdRef.current && !isDesignMode) {
  ...
  }, [connected, conn, identity, sendInput, isDesignMode]);

  // --- Design Mode Effect - Manage input listeners and pointer lock ---
  useEffect(() => {
    if (isDesignMode) {
      // Entering design mode
      console.log("Entering design mode - removing input listeners");
      if (document.pointerLockElement === document.body) {
        try {
          document.exitPointerLock();
        } catch (error) {
          // Handle the case where the user has already manually exited pointer lock
          console.log("Pointer lock already exited by user:", error);
        }
      }
      // Remove input listeners to prevent conflicts
      removeInputListeners();
      removeDelegatedListeners();
    } else if (connected) {
      // Not in design mode and connected - set up input listeners
      console.log(
        "Not in design mode and connected - setting up input listeners"
      );
      setupInputListeners();
  ...
  useEffect(() => {
    ...
    if (conn) {
      ...
      // Don't set up input listeners here - let the design mode effect handle it
      return;
    }
    ...
    console.log(
      `Connecting to SpacetimeDB at ${dbHost}, database: ${dbName}...`
    );
  ...
  return (
    ...
    >
      {showJoinDialog && !isDesignMode && (
        <JoinGameDialog onJoin={handleJoinGame} />
      )}

      {/* Design Mode Toggle Button - Always available */}
      {
        <div
          style={{
            position: "fixed",
        ...
        >
          <button
            ...
            style={{
              marginRight: "10px",
              backgroundColor: isDesignMode ? "#4CAF50" : "#2196F3",
              color: "white",
              border: "none",
              padding: "10px 15px",
              borderRadius: "5px",
          ...
          >
            {isDesignMode ? "🎮 Play Mode" : "🔧 Design Mode"}
          </button>
        </div>
      }
      ...
      {isDesignMode && (
        <DesignModeUI
          showAssetPalette={designModeState.showAssetPalette}
          setShowAssetPalette={designModeState.setShowAssetPalette}
          selectedAsset={designModeState.selectedAsset}
          setSelectedAsset={designModeState.setSelectedAsset}
          transformMode={designModeState.transformMode}
          setTransformMode={designModeState.setTransformMode}
          selectedObjectId={designModeState.selectedObjectId}
          placedObjects={designModeState.placedObjects}
      ...
      {connected && (
        <DebugPanel
          ...
          onToggleExpanded={() =>
            setIsDebugPanelExpanded((prev: boolean) => !prev)
          }
          isPointerLocked={isPointerLocked} // Pass pointer lock state down
        />
      )}

      {/* Always render GameScene and PlayerUI when connected, or in design mode */}
      {(connected || isDesignMode) && (
        <>
          <GameScene
            players={players}
            ...
            designState={designModeState}
          />
          {/* Render PlayerUI only if localPlayer exists and not in design mode */}
          {localPlayer && !isDesignMode && (
            <PlayerUI playerData={localPlayer} />
          )}
        </>
      )}

      {/* Show status when not connected and not in design mode */}
      {!connected && !isDesignMode && (
        <div
          style={{
            display: "flex",
